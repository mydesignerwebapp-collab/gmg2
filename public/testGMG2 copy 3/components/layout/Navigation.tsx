...
export function Navigation() {
  ...
  return (
    <nav className={cn(
      ...
      scrolled 
        ? 'bg-gray-950/98 backdrop-blur-nav shadow-nav' 
        : 'bg-gray-950/95 backdrop-blur-nav'
    )}>
      <div className="container-custom">
        ...
        <div className="flex items-center justify-between py-4">
          ...
          <div className="hidden lg:flex items-center gap-8">
            <a 
              href="tel:1300GMGFIN" 
              className="flex items-center gap-2 hover:text-teal-300 transition-colors text-white text-sm"
            ...
            {navigation.map((item) => (
              <button
                ...
                className="text-white font-medium transition-colors duration-300 hover:text-teal-300"
              >
                {item.name}
              </button>
            ))}
              ...
              <div className="flex flex-col gap-4">
                {navigation.map((item) => (
                  <button
                    key={item.name}
                    onClick={() => handleNavClick(item.href)}
                    className="text-left text-lg font-medium text-white hover:text-teal-300 transition-colors py-2"
                  >
                    {item.name}
                  </button>
                ))}
